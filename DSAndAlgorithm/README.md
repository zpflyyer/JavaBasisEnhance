...
### 7. 2-3树（左偏红黑树）和2-3-4树（普通红黑树）
 - 2-3树无法自顶向下拆分2元素节点，否则退化成了2叉树，因此当插入一个元素后，插入位置如果之前就是一个2元素节点，那么就需要会推举一个红色节点上去，这种情况最多会一直持续到根导致根分裂，或者在到达根之前在某个1元素节点处终止；
 - 2-3-4树自底向上的情形和2-3树差不多：
      - 当待插入位置只有1个元素时，二者相同；
      - 当待插入位置有2个元素时，插入后经过可能的一些调整，最终都会变成一个**‘红黑红’**的3节点，不同的是**2-3树会会将黑色节点推举上去，而2-3-4树是允许这样的3元素节点存在的**。
      - 对于2-3-4树，如果待插入位置有3个元素，那么黑色节点会被推举上去，操作和2-3树的3元素节点向上推举一样。
 - 自底向上的问题在于：无法**并发插入元素**，但只要**将回溯的过程消除掉**，那么只要两个线程操作的是一个节点的不同子树，这些线程之间就不存在竞争关系。而2-3-4树可以通过在递归查找插入位置的过程中，先将路径上所有的3元素节点的黑色元素推举上去，再执行递归，从而最终执行插入的位置元素数量最多只有两个，即**插入路径上所有节点都退化为2-3树里的节点情形**，插入完成后不需要回溯，因为不会有节点被推举上去。
